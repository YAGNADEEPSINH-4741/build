...
export const InteractiveWavesBackground = ({
  ...
  const isVisibleRef = useRef(false); // start paused until visible
  ...
  useEffect(() => {
    ...
    function tick(t) {
      if (!container) return;
      // Skip rendering when not visible in viewport
      if (!isVisibleRef.current) { frameIdRef.current = requestAnimationFrame(tick); return; }
  ...
  return (
    <div ref={containerRef} style={{ backgroundColor, ...style }}
      className={`absolute top-0 left-0 w-full h-full overflow-hidden ${className}`}>
      <div className="absolute top-0 left-0 bg-primary/30 rounded-full w-[0.8rem] h-[0.8rem] pointer-events-none blur-[2px]"
